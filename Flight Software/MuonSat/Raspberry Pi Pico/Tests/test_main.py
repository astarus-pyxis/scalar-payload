...
# Les données du GPS sont enregistrées sur la carte SD, dans le fichier data.csv.
file = open('/sd/data.csv', 'a')
file.write("execution_time, WGS84 _latitude_parser, WGS84_latitude, WGS84_longitude_parser, WGS84_longitude, UTC_timestamp, Altitude\n")    
...
while loop < max_loop:
    ...
    if gnss_l76b.uart_any():
        ...
        if sentence:
            ...
            file.write(str(execution_time) + "," + str(parser.latitude[1]) + "," + str(parser.latitude[0]) + "," + str(parser.longitude[1]) + "," + str(parser.longitude[0]) + "," + str(parser.timestamp[0]) + ":" + str(parser.timestamp[1]) + ":" + str(parser.timestamp[2]) + "," + str(parser.altitude) + "\n")
